#' LDcnv_lrr
#' This function uses lrr intensities
...
#' @export
LDcnv_lrr=function(lrr,map,alpha=0.01,smooth=TRUE,thre=10,dis.thre=5,outname){
  lrr=data.matrix(lrr)
  if(smooth==TRUE){
    lrr <- modSaRa::smooth(lrr, R = 10, t = 2)
  }else{
    lrr=lrr
  }
  h=5
  ...
  fInverse <-
    function(n = 10000, h= 10, hh = 2 * h, precise = 10000, emp,simT = 100){       
      empirical = NULL
      for (i in 1 : simT){
        Y=vector()
        for(p in 1:n){
  ...
  cp <-  LDCNVout(lrr=lrr,map=map, FINV=FINV, alpha =alpha,thre=thre,dis.thre=dis.thre,  outname = outname)$cp
